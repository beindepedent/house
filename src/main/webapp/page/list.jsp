...
<META name=GENERATOR content="MSHTML 8.00.7601.17514">
  ...
  <script language="JavaScript">
    ...
    function goPage(pageNum) {
      //1、将页码设置到表单
      $("#setPage").val(pageNum);
      var d= $("#distric_id").val();
      var s=$("#street_id").val();
      alert("区域："+d+"街道："+s);
      //2、提交表单
      $("#sform").submit();//js提交表单，相当于点击了提交按钮，重新提交了搜索条件
    };
   /* function goDetail(data) {
      $.post("getSingleHouse2",{"id":data})
    }*/
  </script>
</HEAD>
<BODY>
...
<DL class="search clearfix">
  <FORM id=sform method=post action=getBroswerHouse style="width: 1500px">
    <input type="hidden" id="setPage" name="page" value="1">
    标题：<INPUT class=text type=text name=title value="${condition.title}"style="width: 80px">
    区域:<select id="district_id"  name="districtid">
    <option value="">请选择</option>
  </select>
    街道:<select id="street_id" name="streetid">
    <option value="">请选择</option>
  </select>
    类型:<select id="type_id" name="typeid">
    <option value="">请选择</option>
  </select>
    价格:<input type="text" name="startPrice" size="7" value="${condition.startPrice}">
        -<input name="endPrice" type="text" size="7" value="${condition.endPrice}">
    面积: <input type="text" name="startArea" size="7" id="startArea" value="${condition.startArea}"/>
        -<input type="text" size="7" name="endArea"id="endArea" value="${condition.endArea}">
...
<DIV class=pager>
  <TABLE class=house-list>
    ...
    <c:forEach items="${pageInfo.list}" var="h">
      <TR>
        <TD class=house-thumb><span><A href="getSingleHouse2?id=${h.id}" target="_blank"><img src="../images/thumb_house.gif" width="100" height="75" alt=""></a></span></TD>
        <TD>
          <DL>
            <DT><A href="getSingleHouse2?id=${h.id}" target="_blank">${h.title}</A></DT>
            <DD>${h.dname}的${h.sname},${h.floorage}平米<BR>联系方式：${h.contact}</DD></DL></TD>
...
<UL>
  ...
  <LI><A href="javascript:goPage(${pageInfo.prePage==0?pageInfo.pages:pageInfo.nextPage})">下一页</A></LI>
